...
function Gauge({ score, color }) {
  ...
  const data = {
    datasets: [{
      ...
      borderWidth: 0, borderRadius: animated > 2 ? 6 : 0,
...
export default function RiskDashboard({ result, form }) {
  ...
  const factorData = {
    labels,
    datasets: [{
      data: labels.map((r) => Math.max(factorWeights[Object.keys(factorWeights).find(k => r.startsWith(k.split(" (")[0])) ] || 30, 8)),
  ...
  const splitData = {
    labels: ["Default risk", "Repayment"],
    datasets: [{ data: [probPct, 100 - probPct], backgroundColor: [color, "rgba(128,128,128,0.18)"], borderWidth: 0, cutout: "68%" }],
  };
